...
const generateReportPDF = (data) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      // ============================================
      // PATIENT INFO SECTION - Two column layout
      // ============================================
      ...
      doc.font('Helvetica-Bold')
         .fontSize(10)
         .fillColor('#000000')
         .text('AGE :', ageX, currentY, { continued: true });
      doc.font('Helvetica')
         .text(` ${data.patient?.age || 'N/A'} Yrs.`, { continued: true });

      doc.font('Helvetica-Bold')
         .text('SEX :', sexX, currentY, { continued: true });
      doc.font('Helvetica')
         .text(` ${data.patient?.gender || 'MALE'}`, { continued: true });

      doc.font('Helvetica-Bold')
         .text('LAB NO :', labNoX, currentY, { continued: true });
      doc.font('Helvetica')
      ...
      const col5 = 480;     // (empty for alignment)

      // Table Header Background - Light gray
      doc.rect(col1, tableTop, 500, 22)
         .fill('#e8e8e8');
